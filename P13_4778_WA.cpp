...
vector<int> solve(int m, int n, vector<vector<int>> a) {
  ...
  vector<int>ans(TILES + 1, 0);
  ans[0] = 1;

  vector<vector<vector<int>>>dp(2, vector<vector<int>>(STATE, vector<int>(TILES + 1, 0)));

  int prev = 0, cur = 1;
  for(int j = 0; j < n; j++){//col
    for(int i = 0; i < m; i++){//row
      fill(dp[cur].begin(), dp[cur].end(), vector<int>(TILES + 1, 0));
  ...
  for(int tile = 1; tile <= TILES; tile++){
    for(int mask = 0; mask < STATE; mask++){
      ans[tile] += dp[prev][mask][tile];
      if(ans[tile] >= MOD) ans[tile] -= MOD;
    }
  }
