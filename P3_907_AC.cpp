...
void check(vector<int>&row_check, vector<int>&col_check, const int K, vector<vector<pii>>lists, set<vector<pii>>&boards) {
    for (int i = 0; i < K; i++) {
        if (!row_check[i] || !col_check[i]) return;
    }

	for(int i = 0; i < 8; i++){
		sort(lists[i].begin(), lists[i].end());
		if(boards.find(lists[i]) != boards.end()) return;
	}
