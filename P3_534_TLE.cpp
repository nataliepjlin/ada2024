...
const int parent[] = {DOWN, LEFT, UP, RIGHT};
...
void fill_neighbor(vector<vector<string>>&boards, vector<string> &board, 
...
void find_first(vector<vector<string>>&boards, vector<string> &board, 
...
vector<pii> rep_a_to_b(vector<string> &board, const int K){
    vector<pii>b;
    for(int i = 0; i < K; i++){
        for(int j = 0; j < K; j++){
            if(board[i][j] == '1'){
...
int count_polyominoes(int n, int K, std::vector<std::string> s) {
	...
	for(int i = 0; i < boards_size; i++){
		if(skip[i]) continue;
		vector<pii> b = rep_a_to_b(boards[i], K);
		const int to_check = b.size();
