...
vector<int> solve(int m, int n, vector<vector<int>> a) {
  ...
  for(int j = 0; j < n; j++){//col
    for(int i = 0; i < m; i++){//row
      ...
      if(j == 0) dp[prev][(1 << (i + 1)) - 1][0] = 1;// 00111, 01111, etc
      else dp[prev][STATE - 1][0] = 1;

      for(int mask = 0; mask < STATE; mask++){
        for(int tile = 1; tile <= TILES; tile++){
          if(!(mask & (1 << i))){//filled
            if(j > 0 && (a[i][j] != a[i][j - 1])){// horizontal
              dp[cur][mask][tile] += dp[prev][mask ^ (1 << i)][tile - 1];// left is not filled
            }
            if(i > 0 && !(mask & (1 << (i - 1))) && (a[i][j] != a[i - 1][j])){// vertical
              dp[cur][mask][tile] += dp[prev][mask ^ (1 << (i - 1))][tile - 1];// left is filled
              dp[cur][mask][tile] %= MOD;
              dp[cur][mask][tile] += dp[prev][mask ^ (1 << (i - 1)) ^ (1 << i)][tile - 1];// left is empty
              dp[cur][mask][tile] %= MOD;
            }
          }
          else{//empty
            dp[cur][mask][tile] += dp[prev][mask ^ (1 << i)][tile];// left is filled
            dp[cur][mask][tile] += dp[prev][mask][tile];// left is empty
            dp[cur][mask][tile] %= MOD;
          }
        }
      }
