...
int maximum_dishes(std::vector<int>& dishes, int A, int B, long long T){
    ...
    dp[1][1] = A;
    int max_dishes = 1;
    const long long best_case_max = T / A;

    for(int i = 2; i <= n; i++){
        dp[i][1] = A;
        // printf("%lld, ", dp[i][1]);
        for(int j = 2; j <= std::min((long long)i, best_case_max); j++){
            dp[i][j] = T + 1;
            for(int k = j - 1; k < i; k++){
            ...
            printf("%lld, ", dp[i][j]);
            if(dp[i][j] <= T && j > max_dishes){
                max_dishes = j;
                if(max_dishes == best_case_max) return max_dishes;
            }
        }
        printf("\n");
    }
