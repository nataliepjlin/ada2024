...
void find_first(vector<Board>&boards, vector<string>&board, 
 vector<string>&s, const int n, const int K, int fillable) {
	...
	const int stop = (fillable == kk) ? (K >> 1) + 1 : K;//symmetry

	for (int c = 0; c < K; c++) {
		if(c + n > kk || fillable < n) return;
		...
		if (s[0][c] == '.') {
			...
			vector<pair<int, int>>list{{0, c}};
			vector<pair<int, int>>nums_loc{{0, c}};
			fillable--;
			fill_neighbor(boards, board, num_board, nums_loc, 0, c, list, row_check, col_check, s, fillable, n - 1, n, K);
